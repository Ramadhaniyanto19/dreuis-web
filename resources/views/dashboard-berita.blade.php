<x-partial-dashboard>
    <div class="flex flex-col h-full w-full gap-y-3">
        <!-- Breadcrumb -->
        <nav class="flex" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                <li class="inline-flex items-center">
                    <a href="#"
                        class="inline-flex items-center text-lg font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white">
                        <svg class="w-5 h-5 me-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                            fill="currentColor" viewBox="0 0 20 20">
                            <path
                                d="m19.707 9.293-2-2-7-7a1 1 0 0 0-1.414 0l-7 7-2 2a1 1 0 0 0 1.414 1.414L2 10.414V18a2 2 0 0 0 2 2h3a1 1 0 0 0 1-1v-4a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v4a1 1 0 0 0 1 1h3a2 2 0 0 0 2-2v-7.586l.293.293a1 1 0 0 0 1.414-1.414Z" />
                        </svg>
                        Daftar Berita
                    </a>
                </li>
            </ol>
        </nav>

        <!-- Notifikasi -->
        @if (session('success'))
            <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4"
                role="alert">
                <span class="block sm:inline">{{ session('success') }}</span>
            </div>
        @endif
        @if ($errors->any())
            <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4" role="alert">
                <ul>
                    @foreach ($errors->all() as $error)
                        <li>{{ $error }}</li>
                    @endforeach
                </ul>
            </div>
        @endif

        <!-- Search and Create Buttons -->
        <div class="flex justify-between items-center mb-4">
            <form action="{{ route('dashboard-berita.index') }}" method="GET" class="flex items-center gap-2">
                <input type="text" name="search" placeholder="Cari Berita..."
                    class="px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500"
                    value="{{ request('search') }}">
                <button type="submit"
                    class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">Cari</button>
                <a href="{{ route('dashboard-berita.index') }}"
                    class="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600">Reset</a>
            </form>
            <a href="{{ route('dashboard-berita.create') }}"
                class="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600">Tambah Berita</a>
        </div>

        <!-- Tabel Berita -->
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            @if (!empty($beritas) && $beritas->count())
                <table class="w-full text-sm text-gray-500 dark:text-gray-400">
                    <thead class="bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-gray-400">
                        <tr>
                            <th class="px-6 py-3">Gambar</th>
                            <th class="px-6 py-3">Judul</th>
                            <th class="px-6 py-3">Penulis</th>
                            <th class="px-6 py-3">Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($beritas as $berita)
                            <tr class="border-b hover:bg-gray-50 dark:hover:bg-gray-600">
                                <td class="px-6 py-4">
                                    @if ($berita->gambar)
                                        <img src="{{ asset('storage/' . $berita->gambar) }}" alt="{{ $berita->judul }}"
                                            class="w-10 h-10 rounded-full">
                                    @else
                                        <span class="text-gray-400">No Image</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">{{ $berita->judul }}</td>
                                <td class="px-6 py-4">{{ $berita->penulis }}</td>
                                <td class="px-6 py-4 flex gap-2">
                                    <a href="{{ route('dashboard-berita.edit', $berita->id) }}"
                                        class="text-blue-600 hover:underline">Edit</a>
                                    <form action="{{ route('dashboard-berita.destroy', $berita->id) }}" method="POST"
                                        onsubmit="confirmDelete(event)" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-600 hover:underline">Hapus</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            @else
                <div class="p-4 text-center text-gray-700 dark:text-gray-400">Belum ada data berita.</div>
            @endif
        </div>
    </div>
    <div class="mt-4">
        {{ $beritas->links() }}
    </div>

    <!-- Script untuk Alert Konfirmasi Hapus -->
    <script>
        function confirmDelete(event) {
            // Tampilkan alert konfirmasi
            const isConfirmed = confirm('Apakah Anda yakin ingin menghapus data ini?');

            // Jika pengguna memilih "Batal", hentikan proses submit form
            if (!isConfirmed) {
                event.preventDefault(); // Mencegah form submit
            }
        }
    </script>
</x-partial-dashboard>
