<x-partial-dashboard>
    <div class="flex flex-col h-full w-full gap-y-3">
        ...
        <div class="flex justify-between items-center mb-4">
            <form action="{{ route('dashboard-information-rs.index') }}" method="GET" class="flex items-center gap-2">
                <input type="text" name="search" placeholder="Cari Information RS..."
                    class="px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500"
                    value="{{ request('search') }}">
                <button type="submit"
                    class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">Cari</button>
                <a href="{{ route('dashboard-information-rs.index') }}"
        ...
        <div id="crud-modal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden z-50">
            <div class="bg-white p-6 rounded-lg shadow-lg w-96">
                <h2 class="text-lg font-semibold mb-4" id="modal-title">Tambah Data</h2>
                <form id="information-rs-form" action="{{ route('dashboard-information-rs.store') }}" method="POST">
                    @csrf
                    <input type="hidden" id="information-rs-id" name="id">
                    <input type="hidden" name="_method" id="form-method" value="POST"> <!-- Method dinamis -->
                    <div class="mb-4">
                        <label class="block">No. WA</label>
                        <input type="text" id="no_wa" name="no_wa" class="w-full px-4 py-2 border rounded-lg"
                            required>
                    </div>
                    <div class="mb-4">
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            @if (!empty($informationRs) && $informationRs->count())
                    ...
                    <tbody>
                        @foreach ($informationRs as $info)
                            <tr class="border-b hover:bg-gray-50 dark:hover:bg-gray-600">
                                <td class="px-6 py-4">{{ $info->no_wa }}</td>
                                <td class="px-6 py-4">{{ $info->location }}</td>
                                <td class="px-6 py-4 flex gap-2">
                                    <a href="#" class="edit-btn text-blue-600 hover:underline"
                                        data-id="{{ $info->id }}" data-no_wa="{{ $info->no_wa }}"
                                        data-location="{{ $info->location }}">
                                        Edit
                                    </a>
                                    <form action="{{ route('dashboard-information-rs.destroy', $info->id) }}"
            ...
            @else
                <div class="p-4 text-center text-gray-700 dark:text-gray-400">Belum ada data.</div>
            @endif
        </div>
    </div>
    ...
    <script>
        ...
        const form = document.getElementById('information-rs-form');
        const modalTitle = document.getElementById('modal-title');
        const infoIdInput = document.getElementById('information-rs-id');
        ...
        openModalButton.addEventListener('click', () => {
            ...
            form.reset();
            form.setAttribute('action', "{{ route('dashboard-information-rs.store') }}");
            formMethodInput.value = 'POST'; // Set method ke POST
            infoIdInput.value = ''; // Reset ID
        });
        ...
        document.querySelectorAll('.edit-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                const infoId = this.dataset.id;
                const noWa = this.dataset.no_wa;
                const location = this.dataset.location;

                // Isi form dengan data yang ada
                infoIdInput.value = infoId;
                document.getElementById('no_wa').value = noWa;
                document.getElementById('location').value = location;

                // Set action dan method untuk update
                form.setAttribute('action', `/dashboard-information-rs/${infoId}`);
